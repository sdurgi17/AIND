...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if(pre_nodes.issubset(self.s_levels[level])):

                for node_s in self.s_levels[level]:
                    node_s.children.add(action_node)
                    action_node.parents.add(node_s)
